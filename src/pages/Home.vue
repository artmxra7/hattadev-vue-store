  ...
  <section class="section" id="catalog">
    ...
    <div class="container">
      <transition-group name="fadex" class="row row--grid" tag="div">
        <div v-for="item in items" class="col-12 col-sm-6 col-lg-4" :key="item.id">
          <div class="" v-if="currentFilter === item.kategori || currentFilter === 'all'" :key="item.id">
            <div class="product">
              <div class="product__img">
                <img :src="item.foto_url" alt="" />
              </div>
              <h3 class="product__title">{{ item.nama }}</h3>
              <span class="product__price">Rp {{ item.harga }}</span>
  ...
  methods: {
    ...
    addToCart(itemToAdd) {
      ...
      if (this.isAuth()) {
        ...
        formData.append('harga', itemToAdd.harga);
